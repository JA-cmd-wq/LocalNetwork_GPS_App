...
function createAvatarMarker(device: DeviceInfo, selected: boolean): HTMLDivElement {
...
export default function MapView({ devices, selectedEui, showTracks, amapKey, mode, onMapReady }: MapViewProps) {
  ...
  const markerStateRef = useRef<Map<string, { label: string; color: string; selected: boolean }>>(new Map());
  ...
  useEffect(() => {
    ...
    visibleDevices.forEach(device => {
      ...
      if (existing) {
        ...
        if (needContentUpdate) {
          existing.setContent(createAvatarMarker(device, isSelected));
        }
      } else {
        const marker = new AMap.Marker({
          position,
          content: createAvatarMarker(device, isSelected),
          offset: new AMap.Pixel(-20, -24),
        });
        map.add(marker);
        markersRef.current.set(device.eui, marker);
      }
      markerStateRef.current.set(device.eui, { label: device.label, color: device.color, selected: isSelected });
    });
    ...
    const validDevices = visibleDevices.filter(d => d.valid);
